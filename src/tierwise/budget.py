...
Explicitly narrower than a multi-tenant budget/rate-limiting system: this is
one developer's own machine and one number -- "stop letting TierWise touch
routing once I've spent $X this period." It never blocks a request. A
request that would work without TierWise always still goes through; the
ceiling only turns off *enforcement* (the reroute), falling back to
shadow-only (log what would have routed, change nothing) until the period
rolls over. Spend below the ceiling is untouched -- this only ever removes
TierWise's own effect on cost, never adds a new way for a request to fail.
"""
